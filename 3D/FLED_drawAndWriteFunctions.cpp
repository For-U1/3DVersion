...
//using namespace std;
void FLED::writeDetailData(const string & savedatapath)
{
	std::ofstream outfile(savedatapath+"\\"+"DetailAAMED.aamed", std::ios::binary);
...
void FLED::getPointsInfo(vector<vector<float>> &Points) {
	int cols = 6;
	int rows = detEllipses.size();
	vector<vector<float>> arry(rows,vector<float>(cols));
	for (int i = 0;i < detEllipses.size();i++) {
		arry[i][0] = detEllipses[i].center.x;
		arry[i][1] = detEllipses[i].center.y;
		arry[i][2] = detEllipses[i].size.width;
		arry[i][3] = detEllipses[i].size.height;
		arry[i][4] = -detEllipses[i].angle;
		arry[i][5] = detEllipseScore[i];
	}
	Points = arry;
}

void FLED::drawCanndy() {
	cv::namedWindow("imgCanny", cv::WINDOW_NORMAL);
	cv::resizeWindow("imgCanny", 1024, 768);
...
void FLED::drawFLED(Mat ImgG,string savepath)
{
	...
	for (int i = 0; i < detEllipses.size(); i++)
	{
		...
		temp.size.width = detEllipses[i].size.height;
		temp.angle = -detEllipses[i].angle;
		ellipse(Img_T, temp, cv::Scalar(0, 0, 255), 2); //画椭圆
		circle(Img_T, temp.center, 3, cv::Scalar(0, 255, 0), -1);  // 画圆心 B R G;
#if defined(__GNUC__)
	...
	//cv::namedWindow("Ellipses",cv::WINDOW_NORMAL);
	//cv::resizeWindow("Ellipses",1024,768);
	/*cv::imshow("Ellipses", Img_T);*/
	bool success = imwrite(savepath, Img_T);
	if (!success) {
